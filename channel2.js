    ...
    function startPlugin() {  
        ...
        function component(object){  
            ...
            comp.create = function(){  
                ...
                // Глибока копія даних, щоб скинути "горизонтальні" налаштування з головного екрана
                let results = JSON.parse(JSON.stringify(history.slice(0, 50))).map(item => {
                    let ready = item;
                    // Очищуємо опис та примусово вимикаємо широкий режим
                    ready.overview = ''; 
                    ready.is_wide = false;
                    // Повертаємо постер, якщо він був замінений на кадр
                    ready.background_image = item.img || item.poster || item.poster_path;
                    return ready;
                });
                ...
                this.build({  
                    ...
                    line_type: 'poster' // Вертикальні постери для чистоти меню
                });
                
                // Додаємо клас для CSS-фіксів тексту
                this.render().addClass('activity-watched-custom');
        ...
        // Додаємо контент на головний екран (тут залишаємо вашу логіку "як є")
        ...
        // ДОДАЄМО СТИЛІ ДЛЯ ВИПРАВЛЕННЯ НАКЛАДАННЯ ТЕКСТУ
        $('head').append(`<style>
            .activity-watched-custom .card__description,
            .activity-watched-custom .card__content,
            .activity-watched-custom .card__text-boxes {
                display: none !important;
            }
            .activity-watched-custom .card__title {
